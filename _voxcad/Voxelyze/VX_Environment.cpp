...
bool CVX_Environment::ReadXML(CXML_Rip* pXML, std::string* RetMessage) //pXML pointer to an "Environment" element
{
	...
	if (!pXML->FindLoadElement("ContractionOnly", &ContractionOnly)) ContractionOnly = false;
	if (!pXML->FindLoadElement("ExpansionOnly", &ExpansionOnly)) ExpansionOnly = false;
